...
class DeepCeF(deepcf.DeepC_Fragment):
    ...
    def solve_raw(self):
        ...
        upper = np.concatenate((upper, np.array([np.inf] * self.total_length)))
        lower = np.concatenate((lower, np.zeros(self.total_length)))
        upper = np.concatenate((upper, np.array([0.01])))
        lower = np.concatenate((lower, np.array([0.0])))
        ...
        # E0 Matrix
        E0 =  np.concatenate((np.zeros(self.N * self.finish_length), np.zeros(self.init_length), np.ones(self.finish_length))) # Hardcoded SISO
        ...
        D0_sparse = sparse.csc_matrix(D0)
        B1_sparse = sparse.csc_matrix(B1)
        self.solver.setup(D0_sparse, E0, B1_sparse, lower, upper, verbose=False, polish=True)
        results = self.solver.solve()
