...
% Sim setup
simlength_tr = 100; % number of training samples
...
for damping_index = 1:4
    ...
    for segmented = [0, 1] % First run is unsegmeneted, second run is segmented
        ...
        if segmented == 1
            disp("Run Segmented with Damping="+num2str(damping))
        else
            disp("Run Original with Damping="+num2str(damping))
        end
        for rseed = Slist % Run for different realisations of random components
            ...
            trajectories(damping_index+(4*segmented), rseed) = sum(err);
...
save("pendulum_violin", "trajectories")
